...
/**
 * @brief Game4Widget::initial
 * 初始化关卡
 */
void Game4Widget::initial(){
    //背景
    QPalette palette;
    QPixmap pixmap("D:/Qt/Final_Project/GameLevel4/uiProject/gameBackground.png");
    ...
    for(int i = 0; i < 8; i++){
        for(int j = 0; j < 8; j++){
            ...
            this->buttons[i][j] = new MyButton(this, j, i, rand() % 5, isCircleImg);
...
/**
 * @brief MainWindow::moveBtn: 将Btn从start移动到end
 * @param btn button数组
 * @param start 开始位置
 * @param end 结束位置
 * @param duration: animation的参数
 */
void Game4Widget::moveBtn(MyButton *btn, QPoint start, QPoint end, int duration){
 ...
 * @param btn： 槽函数的参数
 */
void Game4Widget::slotWhich2Change(MyButton * btn){
//    QtConcurrent::run([=](){
 ...
 */
void Game4Widget::paintEvent(QPaintEvent *){
 ...
 *         false: 不可以交换
 */
bool Game4Widget::checkIfCanBeChanged(){
    int x1 = this->changePosVec[0]->x, x2 = this->changePosVec[1]->x;
    int y1 = this->changePosVec[0]->y, y2 = this->changePosVec[1]->y;
...
/**
 * @brief MainWindow::checkCurGUI: 检查当前界面是否有可以消去的, 如果有，将所有的icon替换为-1
 * @return
 */
bool Game4Widget::checkCurGUI(){
    flag = false;
    int allToErase = 0;
    for(int i = 0; i < 8; i++){
        for(int j = 0; j < 8; j++){
...
//void GameWidget::playMusic(QString s, bool autoPlay){
//    QtConcurrent::run([=](){
//        QMediaPlayer *myPlayer= new QMediaPlayer;
//        if(!autoPlay)
...
//            playlist->addMedia(QUrl::fromLocalFile("E://uiProject//mp3//" + s));
