 ...
 * @param parent
 */
gameFour::gameFour(QWidget *parent): QWidget(parent), ui(new Ui::gameFour){
    ui->setupUi(this);
    //设置随机数种子
    ...
    for(int i = 0; i < 8; i++){
        for(int j = 0; j < 8; j++){
            this->labels[i][j] = new MyLabel(this, j ,i, rand() % 2);
    ...
    ui->iceLabel->setText(QString::fromStdString(std::to_string(numOfIce)));
...
void gameFour::on_endGameBtn_clicked(){
...
void gameFour::on_progressBtn_clicked(){
    if(this->score < 5000){
 ...
 */
void gameFour::moveBtn(MyButton *btn, QPoint start, QPoint end, int duration){
 ...
 * @param btn： 槽函数的参数
 */
void gameFour::slotWhich2Change(MyButton * btn){
    QtConcurrent::run([=](){
        ...
        for(int i = 0; i < 8; i++)
            for(int j = 0; j < 8; j++)
                if (labels[i][j]->iceNum > 0 )
                    this->numOfIce++;
        ui->iceLabel->setText(QString::number(numOfIce));
            ...
            for(int i = 0; i < 8; i++){
                for(int j = 0; j < 8; j++){
                    ...
                    if(buttons[i][j]->flag == 1){
                        ...
                        buttons[i][j]->reset(buttons[i][j]->flag, -1, 1);
                    }
                }
            }
        }
 ...
 */
void gameFour::paintEvent(QPaintEvent *){
 ...
 *         false: 不可以交换
 */
bool gameFour::checkIfCanBeChanged(){
    int x1 = this->changePosVec[0]->x, x2 = this->changePosVec[1]->x;
    int y1 = this->changePosVec[0]->y, y2 = this->changePosVec[1]->y;
...
void gameFour::on_pauseBtn_clicked(){
    playMusic("popWindow.mp3");
    PauseWidget * pauseWidget = new PauseWidget(this);
...
void gameFour::playMusic(QString s){
    QtConcurrent::run([=](){
        QMediaPlayer *myPlayer= new QMediaPlayer;
...
void gameFour::on_pushButton_clicked(){
    if(this->flush){
        for(int i = 0; i < 8; i++)
            for(int j = 0; j < 8; j++)
                buttons[i][j]->reset(1);
...
void gameFour::closeEvent(QCloseEvent* ){
    myPlayer->stop();
}
