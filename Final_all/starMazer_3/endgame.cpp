...
endGame::endGame(int victflag, QWidget *parent) :QWidget(parent),ui(new Ui::endGame){
    ...
    //setButtonLabel();
    QPixmap pix("E://uiProject///buttonLabel.png");
    buttonLabel->move(140, 460);

    buttonLabel->setFixedSize(110,120);
    buttonLabel->setScaledContents(true);
    pix = pix.scaled(bonusLabel->width(), bonusLabel->height());
    buttonLabel->setPixmap(pix);
    buttonLabel->show();
}
...
void endGame::setButton(){
    ...
    connect(button, SIGNAL(clicked()), this, SLOT(deleteEnd()));
}
