...
Widget::Widget(QWidget *parent): QWidget(parent), ui(new Ui::Widget){
    ...
    //设置背景
    QPalette palette;
    QPixmap pixmap("D:/Qt/Final_Project3/Game2/uiProject/gameBackground.png");
    ...
    buttons[4][4]->reset(0,6);
    this->buttons[4][4]->move(this->x0 + 4 * 65, this->y0 + 4 * 63);
    disconnect(this->buttons[4][4], 0, 0, 0);

    this->buttons[4][4]->setStyleSheet("border:none");

    //设置暂停按钮
    ui->pauseBtn->setStyleSheet("border:none;border-radius:10px;");
    ui->pauseBtn->setIcon(QIcon(QString("D:/Qt/Final_Project3/Game2/uiProject/pauseBtn.png")));
...
void Widget::slotWhich2Change(MyButton * btn){
    QtConcurrent::run([=](){
        ...
        myPlayer->setMedia(QUrl::fromLocalFile("D:/Qt/Final_Project3/Game2/uiProject/click.mp3"));
    ...
    for(int i = 0; i < 8; i++){
        for(int j = 0; j < 8; j++){
            if(buttons[i][j]->isErased != -1){
                if(i == eggPosx && j == eggPosy){
                    moveBtn(buttons[i][j], buttons[i][j]->pos(), QPoint(this->x0 + eggPosx * 65, this->y0 + eggPosx * 63), curSpeed);
                }else{
                    moveBtn(buttons[i][j], buttons[i][j]->pos(), QPoint(this->x0 + buttons[i][j]->x * 65, this->y0 + buttons[i][j]->y * 63), curSpeed);
                    buttons[5][4]->reset(0,5);
...
void Widget::playMusic(QString s){
    QtConcurrent::run([=](){
        QMediaPlayer *myPlayer= new QMediaPlayer;
        myPlayer->setMedia(QUrl::fromLocalFile("D:/Qt/Final_Project3/Game2/uiProject/" + s));
