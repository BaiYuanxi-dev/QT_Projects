...
mianWindow::mianWindow(QWidget *parent): QWidget(parent), ui(new Ui::mainWindow){
    ui->setupUi(this);
    QPalette palette;
    ...
    //设置槽函数
    connect(ui->classOne,&QPushButton::clicked,this,&mianWindow::deal_classOne_Button);
    connect(ui->helpButton,&QPushButton::clicked,this,&mianWindow::deal_help_Button);
}
...
void mianWindow::deal_classOne_Button(){
    emit this->enterGameWid();
    //Widget * w = new Widget();
    //w->show();

}

void mianWindow::deal_help_Button(){
    helpFo.show();
}
