...
Graph::Graph(const char* fileName) {
    ...
    db.close();
    list1 = new int[numOfNodes];


    paths = new int*[100];
    for(int i = 0;i < 100;i ++){
        paths[i] = new int[numOfNodes];
    }
...
int* Graph::getshortestPath(int initIndex, int endIndex, int &num){
...
int** Graph::DFS(int initIndex, int &nums){
    //初始化visited数组
    for(int i = 0;i < numOfNodes;i ++){
        visited[i] = false;
    }
