...
Widget::Widget(QWidget *parent): QWidget(parent), ui(new Ui::Widget){
    ...
    //设置背景
    QPalette palette;
    QPixmap pixmap("D:/Qt/Final_change/Game/uiProject/gameBackground.png");
    ...
    ui->pauseBtn->setIcon(QIcon(QString("D:/Qt/Final_change/Game/uiProject/pauseBtn.png")));
    ui->pauseBtn->setIconSize(QSize(64, 62));

    //设置剩余豆子按钮
    ui->beanBtn->setStyleSheet("border:none;border-radius:10px;");
    ui->beanBtn->setIcon(QIcon(QString("D:/Qt/Final_change/Game/uiProject/bigBox.png")));
    ui->beanBtn->setIconSize(QSize(80, 80));
    ...
    ui->stepLefts->setFont(font);
    ui->stepLefts->setStyleSheet("color:#ff6600;");
    ui->stepLefts->setText(QString(QString::fromStdString(to_string(this->stepLeft))));

    int countBeans=0;
    //计算图有几个beans
    for(int i = 0; i < 8; i++){
        for(int j = 0; j < 8 ; j++){
            if(buttons[i][j]->icon==6){
                countBeans++;
            }
        }
    }
    ...
    ui->beanLeft->setText(QString(QString::fromStdString(to_string(this->beans))));

    ui->leftBeans->setFont(font);
    ui->leftBeans->setStyleSheet("color:#ff6600;");
    ui->leftBeans->setText(QString(QString::fromStdString("五步规则:")));
...
void Widget::on_progressBtn_clicked(){
    if(this->score < 500){
        ...
        QMessageBox::critical(this,"游戏情况","挑战失败！");
    }
    else{
...
bool Widget::eraseBoxes(){
    ...
    for(int i = 0; i < 8; i++ ){
        if(buttons[7][i]->icon==6){
            this->buttons[7][i]->isErased = -1;
            tag = 1;
            this->beans--;
            ui->beanLeft->setText(QString(QString::fromStdString(to_string(this->beans))));
            stepLeft=0;
...
bool Widget::checkIfCanBeChanged(){
    ...
    for(int i = x1 + 1; i < 8; i++){
        if(buttons[y1][i]->icon == buttons[y1][x1]->icon&&buttons[y1][x1]->icon!=6) xx++;
        else break;
    }
    ...
    for(int i = y1 + 1; i < 8; i++){
        if(buttons[i][x1]->icon == buttons[y1][x1]->icon&&buttons[y1][x1]->icon!=6) yy++;
        else break;
    }
    ...
    for(int i = x2 + 1; i < 8; i++){
        if(buttons[y2][i]->icon == buttons[y2][x2]->icon&&buttons[y2][x2]->icon!=6) xx++;
        else break;
    }
    ...
    for(int i = y2 + 1; i < 8; i++){
        if(buttons[i][x2]->icon == buttons[y2][x2]->icon&&buttons[y2][x2]->icon!=6) yy++;
        else break;
    }
...
bool Widget::checkCurGUI(){
    ...
    for(int i = 0; i < 8; i++){
        for(int j = 0; j < 8; j++){
            ...
            //右向检查
            for(int k = j + 1; k < 8; k++){
                if(this->buttons[i][k]->icon == curIcon &&this->buttons[i][k]->icon!=6) x++;
                else break;
            }
            //左向检查
            for(int k = j - 1; k >= 0; k--){
            ...
            //下向检查
            for(int k = i + 1; k < 8; k++){
                if(this->buttons[k][j]->icon == curIcon&&this->buttons[k][j]->icon!=6) y++;
                else break;
            }
            //上向检查
            for(int k = i - 1; k >= 0; k--){
...
void Widget::on_pushButton_clicked(){
    ...
    this->beans=countBeans;
    ui->beanLeft->setStyleSheet("color:#ff6600;");
    ui->beanLeft->setText(QString(QString::fromStdString(to_string(this->beans))));
}

bool Widget::judgeBean(int i,int j,int *store1,int *store2){
    for(int m=0;m<12;m++){
