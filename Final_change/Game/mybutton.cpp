...
MyButton::MyButton(QWidget *parent, int x, int y, int icon):QPushButton(parent){
    ...
    this->isErased = 0;
    this->icon = icon;
    this->setIcon(QIcon(QString("D:/Qt/Final_change/Game/uiProject/box%1.png").arg(this->icon)));
...
void MyButton::reset(int flag, int icon){
    ...
    if(icon != -1){
        this->isErased = 0;
        this->icon = icon;
        this->setIcon(QIcon(QString("D:/Qt/Final_change/Game/uiProject/box%1.png").arg(icon)));
    }
}
